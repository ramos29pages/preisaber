...
export default function ModelManager() {
  ...
  const handleFileChange = (e) => {
    ...
    if (selected && /\.p$|\.pickle$/.test(selected.name)) {
      ...
      setFile(null);
      setFileName("");
      setErrors(prev => ({ ...prev, file: 'Solo archivos .p o .pickle permitidos' }));
      e.target.value = null;
    }
  };
  ...
  const validateForm = () => {
    ...
    } else if (isNaN(accuracy) || parseFloat(accuracy) < 0 || parseFloat(accuracy) > 100) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // De lo contrario:
      alert("Error al crear el modelo: " + (error.message || "Ocurrió un error desconocido"));
    } finally {
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-orange-600">Modelos de Predicción</h2>
          <button
        ...
        <div className="bg-white rounded-xl shadow-md overflow-hidden">
                  ...
                  >
                    <div className="flex items-center">
                      ...
                      {sortConfig.key === 'name' && (
                        <FontAwesomeIcon 
                          icon={faSort} 
                          className={`ml-1 text-orange-500 ${sortConfig.direction === 'ascending' ? 'transform rotate-180' : ''}`}
                        />
                      )}
                    </div>
                  </th>
                ...
                {sortedModels.length > 0 ? (
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <div className="ml-4">
                            ...
                            <div className="text-xs text-gray-500">{model.id ? `ID: ${model.id.substring(0, 8)}...` : ''}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          ...
                          <span className="ml-3 text-sm text-gray-900">{formatAccuracy(model.accuracy)}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{model.uploader}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{formatDate(model.date)}</div>
                      </td>
                  ...
                  <tr>
                    <td colSpan="5" className="px-6 py-10 text-center text-gray-500">
                      {searchTerm ? 'No se encontraron modelos con ese criterio' : <SkeletonModel/>}
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div className="bg-gray-50 px-6 py-3 flex justify-between items-center">
            <div className="text-sm text-gray-500">
              {sortedModels.length} {sortedModels.length === 1 ? 'modelo' : 'modelos'} encontrados
            </div>
            ...
            >
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="space-y-1">
                  ...
                  />
                  {errors.uploader && <p className="text-red-500 text-xs mt-1">{errors.uploader}</p>}
                </div>
                ...
                <div className="space-y-1">
                  <label className="text-sm font-medium text-gray-700 flex items-center gap-2">
                    <FontAwesomeIcon icon={faCloudUploadAlt} className="text-gray-500" />
                    Archivo del modelo
                  </label>
